...
PY_SCRIPTS_DIR="${PY_SCRIPTS_DIR:-${REPO_DIR}/scripts}"; CONFIG_DIR="${CONFIG_DIR:-${REPO_DIR}/configs}"; PATHOGEN_CONFIG_TSV="${PATHOGEN_CONFIG_TSV:-${CONFIG_DIR}/pathogen_panel_3.tsv}"
...
REAL_FASTQ="${REAL_FASTQ:-/home/pthorpe001/data/project_back_up_2024/jcs_blood_samples/MRC1023_AmM008WB.fastq.gz}"; MONKEY_SMALL_GZ="${MONKEY_SMALL_GZ:-/home/pthorpe001/data/2026_plasmodium_kraken_sensitivity/genome/GCF_000956065.1_Mnem_1.0_genomic.fna.gz}"; MONKEY_SMALL_FASTA="${MONKEY_SMALL_FASTA:-/home/pthorpe001/data/2026_plasmodium_kraken_sensitivity/genome/GCF_000956065.1_Mnem_1.0_genomic.fna}"; DEPLETION_REF_FASTA="${DEPLETION_REF_FASTA:-/home/pthorpe001/data/2026_plasmodium_kraken_sensitivity/data/host_plus_plasmo_depletion.fasta}"; DEPLETED_FASTQ="${DEPLETED_FASTQ:-/home/pthorpe001/data/2026_plasmodium_kraken_sensitivity/data/MRC1023_AmM008WB.depleted.fastq.gz}"; KRAKEN_DB_DIR="${KRAKEN_DB_DIR:-/home/pthorpe001/data/project_back_up_2024/kraken_bact_virus_plasmo_fungal}"; OUT_DIR="${OUT_DIR:-/home/pthorpe001/data/2026_plasmodium_kraken_sensitivity/runs/spikein_multi_flye_$(date +%Y%m%d_%H%M%S)}"; THREADS="${THREADS:-12}"; TRAIN_READS_N="${TRAIN_READS_N:-200000}"; SIM_POOL_N="${SIM_POOL_N:-20000}"; SPIKE_LEVELS="${SPIKE_LEVELS:-0 1 5 10 25 50 100 250 500 1000 2500 5000}"; REPLICATES="${REPLICATES:-3}"; DO_HOST_DEPLETION="${DO_HOST_DEPLETION:-true}"
...
GENOME_INDEX=0
for line in "${PANEL_LINES[@]}"; do GENOME_INDEX=$((GENOME_INDEX+1)); pathogen_fasta="$(printf '%s' "${line}" | cut -f1)"; require_file "${pathogen_fasta}"; sim_prefix="${OUT_DIR}/simulated_pathogen_${GENOME_INDEX}"; sim_pool_fastq="${OUT_DIR}/sim_pool_${GENOME_INDEX}.fastq"; sim_pool_fastq_gz="${OUT_DIR}/sim_pool_${GENOME_INDEX}.fastq.gz"; simulator.py genome --ref_g "${pathogen_fasta}" --model_prefix "${NS_MODEL_PREFIX}" --output "${sim_prefix}" --number "${SIM_POOL_N}" --dna_type linear --seed "$((40+GENOME_INDEX))" --num_threads "${THREADS}" --fastq; python3 "${COMBINE_NANOSIM_FASTQ_PY}" --sim_prefix "${sim_prefix}" --out_fastq "${sim_pool_fastq}"; gzip -c "${sim_pool_fastq}" > "${sim_pool_fastq_gz}"; done
...
header='replicate	spike_n	total_spiked_reads	mixed_fastq_gz	flye_out_dir	assembly_fasta	contig_count	total_bases	kraken_report'
...
for rep in $(seq 1 "${REPLICATES}"); do
 for spike_n in ${SPIKE_LEVELS}; do
  ...
  for line in "${PANEL_LINES[@]}"; do target_label="$(printf '%s' "${line}" | cut -f2)"; safe_label="$(printf '%s' "${target_label}" | tr ' ' '_' | tr '/' '_')"; ksum="${MIX_DIR}/assembly.kraken.${safe_label}.summary.tsv"; python3 "${SUMMARISE_KRAKEN_PY}" --kraken_report_tsv "${kraken_report}" --target_label "${target_label}" --out_tsv "${ksum}"; ktarget="$(awk 'NR==2{print $3}' "${ksum}")"; row+="	${ktarget}"; done
  printf '%b\n' "${row}" >> "${SUMMARY_TSV}"
 done
done
