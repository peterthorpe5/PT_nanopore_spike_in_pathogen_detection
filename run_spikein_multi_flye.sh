...
for f in "${REAL_FASTQ}" "${MONKEY_DEPLETION_FASTA}" "${PLASMO_MASKED_GZ}" "${PATHOGEN_CONFIG_TSV}"; do require_file "$f"; done
...
for f in "${SAMPLE_FASTQ_PY}" "${COMBINE_NANOSIM_FASTQ_PY}" "${BUILD_MIXED_FASTQ_PY}" "${SUMMARISE_KRAKEN_PY}" "${DEDUP_FASTQ_NAMES_PY}" "${ASSEMBLY_STATS_PY}"; do require_file "$f"; done
for x in python3 minimap2 samtools gzip kraken2 read_analysis.py simulator.py flye; do require_exe "$x"; done
...

{
  ...
  printf 'pipeline\tmulti_flye\n'
  printf 'pathogen_config_tsv\t%s\n' "${PATHOGEN_CONFIG_TSV}"
  printf 'n_genomes\t%s\n' "${N_GENOMES}"
} > "${RUN_META_TSV}"

header=(replicate spike_n_per_genome n_genomes mixed_fastq_gz mixed_dedup_fastq flye_dir assembly_fasta assembly_contigs assembly_bases kraken_report kraken_classified_contigs)
for label in "${TARGET_LABEL_ARR[@]}"; do header+=("kraken_target_contigs_${label// /_}"); done
(IFS=$'\t'; printf '%s\n' "${header[*]}") > "${SUMMARY_TSV}"

for rep in $(seq 1 "${REPLICATES}"); do
  for spike_n in ${SPIKE_LEVELS}; do
    MIX_DIR="${OUT_DIR}/mix_rep${rep}_n${spike_n}"
    FLYE_DIR="${MIX_DIR}/flye"
    mkdir -p "${MIX_DIR}"
    spike_args=()
    ...
    MIX_DEDUP_FASTQ="${MIX_DIR}/mixed.dedup.fastq"
    ...
    python3 "${DEDUP_FASTQ_NAMES_PY}" --input_fastq_gz "${MIX_FASTQ_GZ}" --output_fastq "${MIX_DEDUP_FASTQ}"
    flye --meta --nano-hq "${MIX_DEDUP_FASTQ}" --out-dir "${FLYE_DIR}" --threads "${THREADS}"

    if [[ -s "${ASSEMBLY_FASTA}" ]]; then
      read -r N_CONTIGS N_BASES < <(python3 "${ASSEMBLY_STATS_PY}" --assembly_fasta "${ASSEMBLY_FASTA}")
      KRAKEN_LABEL_ARGS=()
      ...
      row=("${rep}" "${spike_n}" "${N_GENOMES}" "${MIX_FASTQ_GZ}" "${MIX_DEDUP_FASTQ}" "${FLYE_DIR}" "${ASSEMBLY_FASTA}" "${N_CONTIGS}" "${N_BASES}" "${KRAKEN_REPORT}" "$(awk 'NR==2{print $2}' "${KRAKEN_SUM_TSV}")")
      field_idx=3
      for _ in "${TARGET_LABEL_ARR[@]}"; do row+=("$(awk -v idx="${field_idx}" 'NR==2{print $idx}' "${KRAKEN_SUM_TSV}")"); field_idx=$(( field_idx + 1 )); done
    else
      row=("${rep}" "${spike_n}" "${N_GENOMES}" "${MIX_FASTQ_GZ}" "${MIX_DEDUP_FASTQ}" "${FLYE_DIR}" "${ASSEMBLY_FASTA}" 0 0 "${KRAKEN_REPORT}" 0)
      for _ in "${TARGET_LABEL_ARR[@]}"; do row+=(0); done
    fi
    (IFS=$'\t'; printf '%s\n' "${row[*]}") >> "${SUMMARY_TSV}"
  done
done
