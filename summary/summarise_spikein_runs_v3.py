...
additional classifiers such as MetaMaps and melon, provided their summary
columns follow the existing naming style. For example:

Single read-level examples
--------------------------
- kraken_target_reads
...
Multi read-level examples
-------------------------
- kraken_target_reads_g1
- minimap_target_alignments_g1
- metamaps_target_reads_g1
- melon_target_reads_g1
- melon_estimated_reads_g1
- melon_relative_abundance_g1

Assembly-level examples
-----------------------
- kraken_target_contigs
- kraken_target_contigs_g1
...
Example
-------
python summarise_spikein_runs_updated.py \
...
def standardise_single_assembly(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    dataframe["target_label"] = dataframe.get("target_label", pd.NA)
    dataframe["target_group"] = "single"
    dataframe["assembly_n_contigs"] = dataframe.get("assembly_n_contigs", pd.NA)
    dataframe["assembly_total_bases"] = dataframe.get("assembly_total_bases", pd.NA)
