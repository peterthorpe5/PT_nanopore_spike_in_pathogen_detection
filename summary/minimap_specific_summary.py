...
Tracked-target performance is reported for both alignment-count and
unique-read-count metrics. Real-world summaries are also produced for both
presence modes so that taxonomic cleanliness can be compared under either
measure.
"""
...
def parse_bed_counts(bed_path: Path) -> tuple[dict[str, int], dict[str, int], dict[str, set[str]], int, int]:
...
def discover_mix_dirs(input_roots: Iterable[Path]) -> list[Path]:
...
def build_long_tables(
    *,
    ...
    """Build minimap target-centric and taxon-centric long tables.
    ...
    -------
    tuple[pd.DataFrame, pd.DataFrame]
        Combined long data frame and reported-taxa long data frame.
    """
    combined_rows: list[dict[str, object]] = []
    taxa_rows: list[dict[str, object]] = []

    for mix_dir in mix_dirs:
        bed_path = choose_bed_path(mix_dir)
        if bed_path is None:
            continue
        ...
        combined_rows.append(
            {
                **combined_base,
                "metric": "minimap_total_alignments",
                "target_label": "all_taxa",
                "value": total_alignments,
            }
        )
        combined_rows.append(
            {
                **combined_base,
                "metric": "minimap_total_unique_reads",
                "target_label": "all_taxa",
                "value": total_unique_reads,
            }
        )

        matched_taxa_per_target: dict[str, list[str]] = defaultdict(list)
        ...
        for expected_taxon in expected_taxa:
            ...
            for taxon_name, alignments in taxon_alignments.items():
                if taxon_matches_expected(taxon_name=taxon_name, expected_taxon=expected_taxon):
            ...
            matched_taxa_per_target[expected_taxon] = matched_taxa
            ...
            combined_rows.append(
                {
                    **combined_base,
                    ...
                    "value": expected_alignments,
                }
            )
            combined_rows.append(
                {
                    **combined_base,
                    "metric": "minimap_target_unique_reads",
                    "target_label": expected_taxon,
                    "value": len(expected_read_ids),
                }
            )
            ...
            combined_rows.append(
                {
                    **combined_base,
                    "metric": "minimap_target_unique_reads",
                    "target_label": "total unique",
                    "value": len(total_target_read_ids),
                }
            )
            ...
            taxa_rows.append(
                {
                    **combined_base,
                    ...
                    "is_off_target_taxon": int(bool(expected_taxa) and not bool(matched_expected_labels)),
                }
            )

    combined_df = pd.DataFrame(combined_rows)
    taxa_df = pd.DataFrame(taxa_rows)
    return combined_df, taxa_df
...
def build_observation_level_taxon_presence(
    ...
    *,
    ...
    for group_key, group in taxa_df.groupby(grouping_columns, dropna=False):
        ...
        all_expected = int(n_expected_taxa_present == int(n_expected_taxa) and int(n_expected_taxa) > 0)
        clean_positive = int(bool(all_expected) and n_off_target_taxa == 0)
        any_taxon_negative = int(bool(is_negative) and n_reported_taxa > 0)
        off_target_negative = int(bool(is_negative) and n_off_target_taxa > 0)
        clean_negative = int(bool(is_negative) and n_reported_taxa == 0)
        positive_off_target = int((not bool(is_negative)) and n_off_target_taxa > 0)
        ...
        rows.append(
            {
                ...
                "n_expected_taxa": n_expected_taxa,
...
def summarise_real_world(
    ...
    for group_key, group in observation_df.groupby(grouping_columns, dropna=False):
        key_values = dict(zip(grouping_columns, group_key if isinstance(group_key, tuple) else (group_key,)))
        positives = group.loc[~group["is_negative"]].copy()
        negatives = group.loc[group["is_negative"]].copy()
        ...
        if math.isnan(clean_precision) or math.isnan(clean_sensitivity):
            ...
            clean_f1 = math.nan if denominator == 0 else float(2 * clean_precision * clean_sensitivity / denominator)

        mean_reported_taxa_positive = float(positives["n_reported_taxa"].mean()) if not positives.empty else math.nan
        mean_off_target_taxa_positive = float(positives["n_off_target_taxa"].mean()) if not positives.empty else math.nan
...
def main() -> None:
    ...
    mix_dirs = discover_mix_dirs(input_roots=input_roots)
    if not mix_dirs:
        raise SystemExit("No minimap BED files were found under the input roots.")

    combined_df, taxa_df = build_long_tables(
        mix_dirs=mix_dirs,
    ...
    write_tsv(dataframe=taxa_df, path=out_dir / "minimap_reported_taxa_long.tsv")
    write_tsv(
    ...
    write_tsv(dataframe=summary_df, path=out_dir / "minimap_summary_run_info.tsv")
