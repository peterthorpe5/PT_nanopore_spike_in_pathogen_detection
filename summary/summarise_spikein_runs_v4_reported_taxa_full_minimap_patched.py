...
4. reported_taxa_long.tsv
   Collated long-form table of all reported taxa sidecar outputs for
   off-target and cross-hit analysis.
5. missing_or_problematic_files.tsv
   Missing files, empty files, parse failures, plotting warnings, and
   reported-taxa collation issues.
6. plot_manifest.tsv
...
SUMMARY_FILE_MAP = {
    ...
    "spikein_minimap_only_summary.tsv": "single_read",
    "spikein_multi_minimap_only_summary.tsv": "multi_read",
}
...
NUMERIC_BASE_COLUMNS = [
    ...
    "minimap_target_alignments",
    "minimap_target_unique_reads",
    "minimap_total_alignments",
    "minimap_total_unique_reads",
...
SINGLE_TARGET_METRIC_PATTERN = re.compile(
    ...
    r"(?:_[A-Za-z0-9]+)*"
    r")_(?:reads|alignments|unique_reads|contigs|abundance|clade_reads|direct_reads|found)$"
)

SPECIES_METRIC_PATTERN = re.compile(
    r"^(?P<prefix>kraken|minimap|metamaps|melon|metabuli)_(?P<label>.+)_(?P<suffix>reads|alignments|unique_reads|contigs|abundance|clade_reads|direct_reads|found)$"
)
...
def read_metadata_key_values(metadata_path: Optional[Path]) -> dict[str, str]:
    """Read a two-column run metadata TSV into a dictionary.
...
def infer_single_target_label(row: pd.Series) -> object:
    """Infer a single-target label from row fields or run metadata.
    ...
    -------
    object
        Target label if available, otherwise pandas.NA.
    """
    existing = row.get("target_label", pd.NA)
    if pd.notna(existing) and str(existing).strip():
        return existing

    metadata_path_value = row.get("run_metadata_path", pd.NA)
    metadata = read_metadata_key_values(
        Path(str(metadata_path_value)).expanduser().resolve()
        if pd.notna(metadata_path_value) and str(metadata_path_value).strip()
        else None
    )
    target_label = metadata.get("target_label", "").strip()
    if target_label:
        return target_label

    target_summary_path = row.get("minimap_target_summary_tsv", pd.NA)
    if pd.notna(target_summary_path) and str(target_summary_path).strip():
        try:
            summary_df = pd.read_csv(
                Path(str(target_summary_path)).expanduser().resolve(),
                sep="	",
                dtype=str,
            )
            if "target_label" in summary_df.columns and not summary_df.empty:
                first_label = str(summary_df.iloc[0].get("target_label", "")).strip()
                if first_label:
                    return first_label
        except Exception:
            pass
...
def metric_name_from_parts(prefix: str, suffix: str) -> Optional[str]:
    ...
    mapping = {
        ...
        ("minimap", "unique_reads"): "minimap_target_unique_reads",
...
def standardise_single_read(dataframe: pd.DataFrame) -> pd.DataFrame:
    ...
    dataframe["target_label"] = dataframe.apply(infer_single_target_label, axis=1)
...
def discover_reported_taxa_files(
    input_dirs: Iterable[str],
    verbose: bool,
) -> list[Path]:
    """
    Recursively discover reported-taxa sidecar TSV files.
    ...
    -------
    list[Path]
        Sorted list of discovered reported-taxa sidecar files.
    """
    found: list[Path] = []
    seen: set[Path] = set()

    patterns = ["*.reported_taxa_long.tsv", "*.reported_taxa.tsv"]
    ...
    log(
        f"[INFO] Discovered {len(found)} reported-taxa sidecar file(s)",
        verbose=verbose,
    )
    return sorted(found)


def build_report_path_metadata_map(combined_wide: pd.DataFrame) -> dict[str, list[dict[str, object]]]:
    """
    Build a mapping from report TSV paths to summary-row metadata.
    ...
    dict[str, list[dict[str, object]]]
        Mapping from resolved report TSV path to associated metadata rows.
    """
    ...
    path_map: dict[str, list[dict[str, object]]] = {}

    for _, row in combined_wide.iterrows():
        base_meta = {column: row.get(column, pd.NA) for column in metadata_columns}
        for column, value in row.items():
            if pd.isna(value):
                continue

            value_str = str(value).strip()
            if not value_str.endswith(".tsv"):
                continue
            if "/" not in value_str and not value_str.startswith("."):
                continue

            resolved_path = str(Path(value_str).expanduser().resolve())
            meta_record = {**base_meta, "matched_from_column": str(column)}
            existing = path_map.setdefault(resolved_path, [])

            if meta_record not in existing:
                existing.append(meta_record)

    return path_map


def empty_reported_taxa_table() -> pd.DataFrame:
    """
    Create an empty reported-taxa long table with standard columns.

    Returns
    -------
    pd.DataFrame
        Empty reported-taxa long table.
    """
    return pd.DataFrame(
        columns=[
            "workflow_type",
...
def collate_reported_taxa_long(
    input_dirs: Iterable[str],
    combined_wide: pd.DataFrame,
    issues: list[IssueRecord],
    verbose: bool,
    ...
    """
    Collate reported-taxa sidecar TSVs into one long table.

    Parameters
    ----------
    input_dirs : Iterable[str]
        Input directories to scan recursively.
    combined_wide : pd.DataFrame
        Combined wide summary table used to recover replicate/run metadata.
    issues : list[IssueRecord]
        Mutable list of non-fatal issues.
    verbose : bool
        Whether verbose logging is enabled.
    ...
    -------
    pd.DataFrame
        Collated reported-taxa long table.
    """
    sidecar_paths = discover_reported_taxa_files(
        input_dirs=input_dirs,
        verbose=verbose,
    )
    if not sidecar_paths:
        return empty_reported_taxa_table()

    report_path_map = build_report_path_metadata_map(combined_wide=combined_wide)
    records: list[dict[str, object]] = []

    for sidecar_path in sidecar_paths:
        sidecar_df = safe_read_tsv(path=sidecar_path, issues=issues)
        if sidecar_df is None:
            continue
        if sidecar_df.empty:
            issues.append(
                IssueRecord(
                    "empty_reported_taxa_table",
                    str(sidecar_path),
                    "Reported-taxa sidecar exists but contains no rows.",
                )
            )
            continue
        ...
        missing_columns = required_columns - set(sidecar_df.columns)
        if missing_columns:
            compact_minimap_columns = {
                "taxon_name",
                "taxid",
                "reference_count",
                "alignments",
            ...
            if compact_minimap_columns.issubset(set(sidecar_df.columns)):
                converted_records: list[dict[str, object]] = []
                resolved_sidecar_path = str(sidecar_path.expanduser().resolve())
                for _, compact_row in sidecar_df.iterrows():
                    common_values = {
                        "classifier": "minimap",
                        "report_tsv": resolved_sidecar_path,
                        "filter_prefix": pd.NA,
                        "matched_target_labels": compact_row.get("matched_target_labels", pd.NA),
                        "taxid": compact_row.get("taxid", pd.NA),
                        "taxon_name": compact_row.get("taxon_name", pd.NA),
                        "rank_code": pd.NA,
                        "percent": pd.NA,
                    }
                    converted_records.append(
                        {
                            **common_values,
                            ...
                            "metric_value": compact_row.get("alignments", pd.NA),
                        }
                    )
                    converted_records.append(
                        {
                            **common_values,
                            ...
                            "metric_value": compact_row.get("unique_reads", pd.NA),
                        }
                    )
                sidecar_df = pd.DataFrame.from_records(converted_records)
            else:
                issues.append(
                    IssueRecord(
                        "reported_taxa_missing_columns",
                        str(sidecar_path),
                        "Missing required reported-taxa columns: "
                        + ", ".join(sorted(missing_columns)),
                    )
                )
                continue

        for _, taxa_row in sidecar_df.iterrows():
            report_tsv_value = taxa_row.get("report_tsv", pd.NA)
            report_matches: list[dict[str, object]] = []

            if pd.notna(report_tsv_value) and str(report_tsv_value).strip():
                resolved_report = str(Path(str(report_tsv_value)).expanduser().resolve())
                report_matches = report_path_map.get(resolved_report, [])
            else:
                resolved_report = pd.NA

            if not report_matches:
                issues.append(
                    IssueRecord(
                        "reported_taxa_unmatched_report",
                        str(sidecar_path),
                        f"No summary-row metadata match for report_tsv={report_tsv_value}",
                    )
                )
                report_matches = [
                    {
                        "workflow_type": pd.NA,
                        "run_name": sidecar_path.parent.name,
                        "run_dir": str(sidecar_path.parent),
                        "summary_path": pd.NA,
                        "is_shuffled_control": pd.NA,
                        "replicate": pd.NA,
            ...
            for match in report_matches:
                records.append(
                    {
                        "workflow_type": match.get("workflow_type", pd.NA),
                        "run_name": match.get("run_name", pd.NA),
                        "run_dir": match.get("run_dir", pd.NA),
                        "summary_path": match.get("summary_path", pd.NA),
                        "is_shuffled_control": match.get("is_shuffled_control", pd.NA),
                        "replicate": match.get("replicate", pd.NA),
                        "spike_n_per_genome": match.get("spike_n_per_genome", pd.NA),
                        "total_spike_n": match.get("total_spike_n", pd.NA),
                        "n_genomes": match.get("n_genomes", pd.NA),
                        "assembly_n_contigs": match.get("assembly_n_contigs", pd.NA),
                        "assembly_total_bases": match.get("assembly_total_bases", pd.NA),
                        "reported_taxa_source_tsv": str(sidecar_path),
                        "matched_from_column": match.get("matched_from_column", pd.NA),
                        "classifier": taxa_row.get("classifier", pd.NA),
                        "report_tsv": resolved_report,
                        "filter_prefix": taxa_row.get("filter_prefix", pd.NA),
                        "matched_target_labels": taxa_row.get("matched_target_labels", pd.NA),
                        "taxid": taxa_row.get("taxid", pd.NA),
                        "taxon_name": taxa_row.get("taxon_name", pd.NA),
                        "rank_code": taxa_row.get("rank_code", pd.NA),
                        "percent": taxa_row.get("percent", pd.NA),
                        "metric_name": taxa_row.get("metric_name", pd.NA),
                        "metric_value": taxa_row.get("metric_value", pd.NA),
                    }
                )
    ...
    reported_taxa_df = pd.DataFrame.from_records(records)
    reported_taxa_df = coerce_numeric_columns(
    ...
    reported_taxa_df = reported_taxa_df.sort_values(
        by=[
            "workflow_type",
            "run_name",
            "replicate",
    ...
    log(
        f"[INFO] Collated {len(reported_taxa_df)} reported-taxa long row(s)",
        verbose=verbose,
    )
    return reported_taxa_df
...
def main() -> None:
    ...
    reported_taxa_long = collate_reported_taxa_long(
        input_dirs=args.input_dirs,
    ...
    write_tsv(reported_taxa_long, out_dir / "reported_taxa_long.tsv")
    ...
    print(f"[INFO] N reported-taxa sidecars parsed: {0 if reported_taxa_long.empty else reported_taxa_long['reported_taxa_source_tsv'].nunique()}")
