...
PATHOGEN_CONFIG_TSV="${PATHOGEN_CONFIG_TSV:-${CONFIG_DIR}/pathogen_panel_3.tsv}"
...
for line in "${PANEL_LINES[@]}"; do target_label="$(printf '%s' "${line}" | cut -f2)"; safe_label="$(printf '%s' "${target_label}" | tr ' ' '_' | tr '/' '_')"; header+="	kraken_${safe_label}_reads	minimap_${safe_label}_alignments"; done
printf '%b\n' "${header}" > "${SUMMARY_TSV}"
...
for rep in $(seq 1 "${REPLICATES}"); do
  for spike_n in ${SPIKE_LEVELS}; do
    ...
    kraken_report="${MIX_DIR}/kraken.report.tsv"; kraken_out="${MIX_DIR}/kraken.classifications.tsv"
    kraken2 --db "${KRAKEN_DB_DIR}" --threads "${THREADS}" --report "${kraken_report}" --output "${kraken_out}" "${mix_fastq_gz}" >/dev/null
    bam_out="${MIX_DIR}/minimap_sorted.bam"; bed_out="${MIX_DIR}/minimap_sorted.MASKED.bed"
