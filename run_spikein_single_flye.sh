...
for f in "${REAL_FASTQ}" "${PATHOGEN_FASTA}" "${MONKEY_DEPLETION_FASTA}" "${PLASMO_MASKED_GZ}"; do require_file "$f"; done
...

{
  ...
  printf 'pipeline\tsingle_flye\n'
  printf 'pathogen_fasta\t%s\n' "${PATHOGEN_FASTA}"
  printf 'target_label\t%s\n' "${TARGET_LABEL}"
} > "${RUN_META_TSV}"

printf 'replicate\tspike_n\tmixed_fastq_gz\tmixed_dedup_fastq\tflye_dir\tassembly_fasta\tassembly_contigs\tassembly_bases\tkraken_report\tkraken_classified_contigs\tkraken_target_contigs\n' > "${SUMMARY_TSV}"
...
for rep in $(seq 1 "${REPLICATES}"); do
  for spike_n in ${SPIKE_LEVELS}; do
    ...
    if [[ -s "${ASSEMBLY_FASTA}" ]]; then
      ...
      N_CONTIGS=0; N_BASES=0; K_CLASSIFIED=0; K_TARGET=0
    fi

    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "${rep}" "${spike_n}" "${MIX_FASTQ_GZ}" "${MIX_DEDUP_FASTQ}" "${FLYE_DIR}" "${ASSEMBLY_FASTA}" "${N_CONTIGS}" "${N_BASES}" "${KRAKEN_REPORT}" "${K_CLASSIFIED}" "${K_TARGET}" >> "${SUMMARY_TSV}"
  done
done
