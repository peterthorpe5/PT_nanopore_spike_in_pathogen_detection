...
RUN_SCRIPT="${RUN_SCRIPT:-${SHELLS_DIR}/run_spikein_shuffled_flye_medaka.sh}"
...
export OUT_DIR="${OUT_DIR:-${PROJECT_DIR}/runs/spikein_shuffle_flye_medaka_rerun_$(date +%Y%m%d_%H%M%S)}"

log_info "Project dir: ${PROJECT_DIR}"
log_info "Repo dir: ${REPO_DIR}"
log_info "Using run script: ${RUN_SCRIPT}"
log_info "Shuffle seed: ${SHUFFLE_SEED}"
log_info "Medaka model: ${MEDAKA_MODEL}"
log_info "Output dir: ${OUT_DIR}"
log_info "Submitting shuffled single-genome Flye plus Medaka assembly rerun"

bash "${RUN_SCRIPT}"

log_info "Shuffled single-genome Flye plus Medaka rerun finished"
log_info "Output dir: ${OUT_DIR}"
