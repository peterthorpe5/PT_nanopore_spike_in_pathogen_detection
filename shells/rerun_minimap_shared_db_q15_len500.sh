...
NEW_RUNS_DIR="${PROJECT_BASE}/runs_minimapfix_shared_q15_len500_${STAMP}"
PATCHED_SHELLS_DIR="${NEW_RUNS_DIR}/patched_shells"
SUMMARY_STAGE_DIR="${PROJECT_BASE}/summary_input_minimapfix_shared_q15_len500_${STAMP}"
SUMMARY_OUT_DIR="${PROJECT_BASE}/spikein_summary_minimapfix_shared_q15_len500_${STAMP}"

mkdir -p "${NEW_RUNS_DIR}" "${PATCHED_SHELLS_DIR}" "${SUMMARY_STAGE_DIR}" "${SUMMARY_OUT_DIR}"

log_info "Using shared benchmark minimap database from pipeline_paths.sh"
log_info "MINIMAP_DB_FASTA=${MINIMAP_DB_FASTA_DEFAULT}"
...
export MINIMAP_DB_FASTA="${MINIMAP_DB_FASTA_DEFAULT}"

log_info "Running single-genome read-level benchmark"
export OUT_DIR="${NEW_RUNS_DIR}/spikein_single_read_minimapfix_shared_${STAMP}"
bash "${PATCHED_SHELLS_DIR}/run_spikein_single_readlevel.sh"

log_info "Running shuffled negative-control read-level benchmark"
export OUT_DIR="${NEW_RUNS_DIR}/spikein_shuffle_read_minimapfix_shared_${STAMP}"
bash "${PATCHED_SHELLS_DIR}/run_spikein_shuffled_readlevel.sh"

log_info "Running two-genome read-level benchmark"
export PATHOGEN_CONFIG_TSV="${REPO_DIR}/configs/pathogen_panel_2.tsv"
export OUT_DIR="${NEW_RUNS_DIR}/spikein_multi_read_panel2_minimapfix_shared_${STAMP}"
bash "${PATCHED_SHELLS_DIR}/run_spikein_multi_readlevel.sh"

log_info "Running three-genome read-level benchmark"
export PATHOGEN_CONFIG_TSV="${REPO_DIR}/configs/pathogen_panel_3.tsv"
export OUT_DIR="${NEW_RUNS_DIR}/spikein_multi_read_panel3_minimapfix_shared_${STAMP}"
bash "${PATCHED_SHELLS_DIR}/run_spikein_multi_readlevel.sh"
...
python3 "${REPO_DIR}/summary/make_spikein_report_v5.py" \
    ...
    --title "ONT spike-in summary report: minimap rerun shared DB Q15 len500"

python3 "${REPO_DIR}/summary/make_spikein_replicate_report_v2.py" \
    --summary_dir "${SUMMARY_OUT_DIR}" \
    --out_dir "${SUMMARY_OUT_DIR}/replicate_resolved_report" \
    --title "ONT spike-in replicate-resolved report: minimap rerun shared DB Q15 len500" \
    --threshold_mode fixed \
    --min_detect_value 1
...
python3 "${REPO_DIR}/summary/make_spikein_threshold_calibration_report_v3.py" \
    ...
    --title "ONT spike-in threshold calibration report: minimap rerun shared DB Q15 len500" \
    --min_detect_value 1 \
    --target_fpr 0.05

log_info "Finished shared-database minimap rerun"
