...
OUT_DIR="${OUT_DIR:-/home/pthorpe001/data/2026_plasmodium_kraken_sensitivity/runs/spikein_single_flye_medaka2_$(date +%Y%m%d_%H%M%S)}"
THREADS="${THREADS:-12}"
...
for x in python3 read_analysis.py simulator.py flye kraken2 gzip minimap2 samtools medaka_consensus; do require_exe "$x"; done
...
for rep in $(seq 1 "${REPLICATES}"); do
  for spike_n in ${SPIKE_LEVELS}; do
    ...
    run_medaka_consensus "${DEDUP_FASTQ}" "${ASSEMBLY_FASTA}" "${MEDAKA_ROUND1_OUT_DIR}" "${THREADS}" "${MEDAKA_MODEL}"
    require_file "${MEDAKA_ROUND1_FASTA}"

    run_medaka_consensus "${DEDUP_FASTQ}" "${MEDAKA_ROUND1_FASTA}" "${MEDAKA_ROUND2_OUT_DIR}" "${THREADS}" "${MEDAKA_MODEL}"
