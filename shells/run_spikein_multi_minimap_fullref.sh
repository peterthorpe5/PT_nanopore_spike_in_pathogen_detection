...
FULL_MINIMAP_DB_FASTA="${FULL_MINIMAP_DB_FASTA:-${MINIMAP_DB_FASTA_DEFAULT:-}}"
[[ -n "${FULL_MINIMAP_DB_FASTA}" ]] || {
    log_error "FULL_MINIMAP_DB_FASTA is empty and MINIMAP_DB_FASTA_DEFAULT was not set in pipeline_paths.sh"
    exit 1
}
require_file "${FULL_MINIMAP_DB_FASTA}"

export MINIMAP_DB_FASTA="${FULL_MINIMAP_DB_FASTA}"
export OUT_DIR="${OUT_DIR:-/home/pthorpe001/data/2026_plasmodium_kraken_sensitivity/runs/spikein_multi_minimap_fullref_$(date +%Y%m%d_%H%M%S)}"
