...
run_medaka_consensus() {
    ...
    if [[ "${model}" == "auto" ]]; then
        log_info "Running Medaka with automatic model selection: ${medaka_out_dir}"
        medaka_consensus \
            -i "${reads_fastq}" \
            -d "${draft_fasta}" \
            -o "${medaka_out_dir}" \
        ...
        log_info "Running Medaka with explicit model ${model}: ${medaka_out_dir}"
        medaka_consensus \
            -i "${reads_fastq}" \
            -d "${draft_fasta}" \
            -o "${medaka_out_dir}" \
...
OUT_DIR="${OUT_DIR:-/home/pthorpe001/data/2026_plasmodium_kraken_sensitivity/runs/spikein_multi_flye_medaka2_$(date +%Y%m%d_%H%M%S)}"
THREADS="${THREADS:-12}"
TRAIN_READS_N="${TRAIN_READS_N:-200000}"
SIM_POOL_N="${SIM_POOL_N:-20000}"
SPIKE_LEVELS="${SPIKE_LEVELS:-${SPIKE_LEVELS_DEFAULT:-0 1 5 10 25 50 100 250 500 1000 2500 5000}}"
REPLICATES="${REPLICATES:-${REPLICATES_DEFAULT:-12}}"
DO_HOST_DEPLETION="${DO_HOST_DEPLETION:-true}"
MEDAKA_MODEL="${MEDAKA_MODEL:-auto}"
MEDAKA_ROUNDS="2"

for executable in python3 read_analysis.py simulator.py flye kraken2 minimap2 samtools medaka_consensus; do
...

{
    ...
    printf 'medaka_model\t%s\n' "${MEDAKA_MODEL}"
    printf 'medaka_rounds\t%s\n' "${MEDAKA_ROUNDS}"
} > "${RUN_META_TSV}"
...
for line in "${PANEL_LINES[@]}"; do
    ...
    header+="\tkraken_${safe_label}_contigs"
done
printf '%b\n' "${header}" > "${SUMMARY_TSV}"
...
for rep in $(seq 1 "${REPLICATES}"); do
    for spike_n in ${SPIKE_LEVELS}; do
        ...
        python3 "${DEDUP_FASTQ_NAMES_PY}" \
            --input_fastq_gz "${mix_fastq_gz}" \
            --output_fastq "${dedup_fastq}"

        flye_out_dir="${MIX_DIR}/flye_out"
        assembly_fasta="${flye_out_dir}/assembly.fasta"
        medaka_round1_out_dir="${MIX_DIR}/medaka_round1_out"
        medaka_round1_fasta="${medaka_round1_out_dir}/consensus.fasta"
        medaka_round2_out_dir="${MIX_DIR}/medaka_round2_out"
        polished_fasta="${medaka_round2_out_dir}/consensus.fasta"
        ...
        require_file "${assembly_fasta}"

        run_medaka_consensus \
            "${dedup_fastq}" \
            "${assembly_fasta}" \
            "${medaka_round1_out_dir}" \
            "${THREADS}" \
            "${MEDAKA_MODEL}"

        require_file "${medaka_round1_fasta}"

        run_medaka_consensus \
            "${dedup_fastq}" \
            "${medaka_round1_fasta}" \
            "${medaka_round2_out_dir}" \
            "${THREADS}" \
            "${MEDAKA_MODEL}"

        require_file "${polished_fasta}"

        python3 "${ASSEMBLY_STATS_PY}" \
            --assembly_fasta "${polished_fasta}" \
            --out_tsv "${asm_stats_tsv}"

        require_file "${asm_stats_tsv}"
