...
python3 "${SHUFFLE_FASTA_PY}" \
    --input_fasta "${PATHOGEN_FASTA}" \
    --output_fasta "${SHUFFLED_FASTA}" \
    --metadata_tsv "${SHUFFLE_META_TSV}" \
    --seed "${SHUFFLE_SEED}"

REPO_DIR="${REPO_DIR}" \
PY_SCRIPTS_DIR="${PY_SCRIPTS_DIR}" \
PATHOGEN_FASTA="${SHUFFLED_FASTA}" \
OUT_DIR="${OUT_DIR}/main_run" \
MEDAKA_MODEL="${MEDAKA_MODEL}" \
bash "${SHELLS_DIR}/run_spikein_single_flye_medaka.sh"
